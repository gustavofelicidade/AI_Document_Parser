...
def analyze_query_fields():
    ...
    poller = document_intelligence_client.begin_analyze_document(
        ...
        query_fields=["CPF",
                    ...
                    "Data_Emissao",
                    "Local",
                    "Doc_Identidade"]  # Set the features and provide a comma-separated list of field names.
    )
    ...
    if result.documents:
        for doc in result.documents:
            # 1
            if doc.fields and doc.fields["CPF"]:
                print(f"CPF: {doc.fields['CPF'].value_string}")
            # 2
            if doc.fields and doc.fields["Validade"]:
                print(f"Validade: {doc.fields['Validade'].value_string}")
            # 3
            if doc.fields and doc.fields["Habilitacao"]:
                print(f"Habilitacao: {doc.fields['Habilitacao'].value_string}")
            # 4
            if doc.fields and doc.fields["CatHab"]:
                print(f"CatHab: {doc.fields['CatHab'].value_string}")
            # 5
            if doc.fields and doc.fields["orgEmissor_UF"]:
                print(f"orgEmissor_UF: {doc.fields['orgEmissor_UF'].value_string}")
            # 6
            if doc.fields and doc.fields["Data_Emissao"]:
                print(f"Data_Emissao: {doc.fields['Data_Emissao'].value_string}")
            # 7
            if doc.fields and doc.fields["Local"]:
                print(f"Local: {doc.fields['Local'].value_string}")
            # 8
            if doc.fields and doc.fields["Doc_Identidade"]:
                print(f"Doc_Identidade: {doc.fields['Doc_Identidade'].value_string}")
            # 9
            if doc.fields and doc.fields["FirstName"]:
                print(f"FirstName: {doc.fields['FirstName'].value_string}")
            # 10
            if doc.fields and doc.fields["LastName"]:
                print(f"LastName: {doc.fields['LastName'].value_string}")
            # 11
            if doc.fields and doc.fields["DateOfBirth"]:
                print(f"DateOfBirth: {doc.fields['DateOfBirth'].value_string}")
            # 12
            if doc.fields and doc.fields["DocumentNumber"]:
                print(f"DocumentNumber: {doc.fields['DocumentNumber'].value_string}")
